...
class CreateJyGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_goods', function (Blueprint $table) {
            ...
            $table->enum("is_shop",[1,2])->default(1)->comment("是否上架 1正常 2禁用");
            $table->timestamp("shop_time")->comment("上架时间");
            $table->enum("is_recommand",[1,2])->default(1)->comment("是否推荐 1是2否");
            $table->enum("is_new",[1,2])->default(1)->comment("是否最新 1是2否");
            $table->enum("is_hot",[1,2])->default(1)->comment("是否热销 1是2否");
            $table->integer("sort")->default(100)->comment("商品排序");
            $table->integer("give_score")->default(rand(1,100))->comment("商品送的积分");
