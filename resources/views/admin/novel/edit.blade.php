    ...
    <div class="contentpanel">
      ...
      <div class="panel panel-default">
        <div class="panel-heading">
          ...
          <h4 class="panel-title">小说章节编辑</h4>
        ...
        <div class="panel-body panel-body-nopadding">
          ...
          <form class="form-horizontal form-bordered" action="{{route('admin.novel.doEdit')}}" method="post" enctype="multipart/form-data">
                ...
                <select name="c_id" class="form-bordered" >
                  @if(!empty($category))
                    @foreach($category as $categorys)
                      <option value="{{$categorys['id']}}" {{$categorys['id'] == $novel->c_id  ? "selected" : "" }}>{{$categorys['c_name']}}</option>
                    @endforeach 
                  @endif
                </select>
              </div>
            ...
            <div class="form-group">
                ...
                <select name="a_id" class="form-bordered" >
                  @if(!empty($author))
                    @foreach($author as $authors)
                      <option value="{{$authors['id']}}" {{$authors['id'] == $novel->a_id  ? "selected" : "" }}>{{$authors['author_name']}}</option>
                    @endforeach 
                  @endif
                </select>
              </div>
             ...
             <div class="form-group">
              ...
              <div class="col-sm-6">
                ...
                <img src="{{$novel->image_url}}" alt="" style="border:red 1px solid;width:60px;">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-6">
                 <div class="radio"><label><input type="radio" value="1" name="status" {{$novel->status == 1 ? 'checked' : ""}} >连载</label></div>
                 <div class="radio"><label><input type="radio"   value="2" name="status"  {{$novel->status == 2 ? 'checked' : ""}} >完结</label></div>
               </div>
            </div>          
            ...
            <div class="form-group">
              ...
              <div class="col-sm-8">
                <textarea name="desc" style="border:none;" id="container" class="form-control" >{{$novel->desc}}</textarea>
              </div>
            </div>
			 ...
			 <div class="row">
				<div class="col-sm-6 col-sm-offset-3">
				  <button class="btn btn-primary btn-danger" id="btn-save">修改小说</button>&nbsp;
